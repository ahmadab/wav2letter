...
int main(int argc, char** argv)
{
	...
	else
	{
		...
		std::string loadPath = pathsConcat(FLAGS_emission_dir,
				cleanedTestPath + ".bin");
	...
	if (FLAGS_emission_dir.empty())
	{
		...
		int worldRank = 0;
		int worldSize = 1;
		auto ds = createDataset(FLAGS_test, dicts, lexicon, 1, worldRank,
				worldSize);
		...
		for (auto& sample : *ds)
		{
			auto rawEmission = network->forward(
			{ fl::input(sample[kInputIdx]) }).front();
	...
	int nSamplePerThread = std::ceil(
			nSample / static_cast<float>(FLAGS_nthread_decoder));
	...
	std::vector<int> sliceAltNumWords(FLAGS_nthread_decoder, 0);
	std::vector<int> sliceAltNumLetters(FLAGS_nthread_decoder, 0);
	std::vector<int> sliceAltNumSamples(FLAGS_nthread_decoder, 0);
	...
	DecoderOptions decoderOpt(FLAGS_beamsize, static_cast<float>(FLAGS_beamscore),
			static_cast<float>(FLAGS_lmweight), static_cast<float>(FLAGS_wordscore),
			static_cast<float>(FLAGS_unkweight), FLAGS_forceendsil, FLAGS_logadd,
			static_cast<float>(FLAGS_silweight), modelType);
	...
	auto runDecoder = [&](int tid, int start, int end)
	{
		try
		{
			...
			// init test meters
			std::vector<TestMeters *> altMeters;
			for (int alt = 0; alt < MaxAlt; alt++)
			{
				TestMeters *meters;
				meters->timer.resume();
				altMeters.push_back(meters);
			}
			...
			for (int s = start; s < end; s++)
			{
				...
				auto wordTargets = emissionSet.wordTargets[s];
				...
				// update meters for all alternates
				for (int alt = 0; alt < MaxAlt, alt < wordPredictions.size(); alt++)
				{
					auto wordPrediction = wordPredictions[alt];
					auto letterPrediction = letterPredictions[alt];
					...
					meters.wer.reset();
					meters.ler.reset();
					meters.wer.add(wordPrediction, wordTargets);
					meters.ler.add(letterPrediction, letterTarget);

					// update the alt meters
					for (int nextAlt = alt; nextAlt < MaxAlt; nextAlt++)
					{
						TestMeters *nextAltMeter = altMeters[nextAlt];

						if (alt == 0)
						{
							nextAltMeter->wer.add(wordPrediction, wordTargets);
							nextAltMeter->ler.add(letterPrediction, letterTarget);
						}
						else
						{
							if (nextAltMeter->wer.value()[0] >= meters.wer.value()[0])
							{
								nextAltMeter->wer.value()[0] = meters.wer.value()[0];
							}

							if (nextAltMeter->ler.value()[0] >= meters.ler.value()[0])
							{
								nextAltMeter->ler.value()[0] = meters.ler.value()[0];
							}
						}
					}
					...
					if (FLAGS_show && alt == 0)
					{
						auto wordTargetStr = tensor2words(wordTargets, wordDict);
						...
						sliceNumWords[tid] += wordTargets.size();
						sliceNumLetters[tid] += letterTarget.size();
						sliceWer[tid] = altMeters[0]->werSlice.value()[0];
						sliceLer[tid] = altMeters[0]->lerSlice.value()[0];
						sliceNumSamples[tid] = sliceSize;
						sliceTime[tid] = meters.timer.value();
					}
				}

				// compute slice test meters for each position
				if (FLAGS_show)
				{
					buffer << "[sample: " << sampleId
					<< ", WER[0]: " << altMeters[0]->wer.value()[0]
					<< "\%, LER[0]: " << altMeters[0]->ler.value()[0]
					<< "\%, slice WER[0]: " << altMeters[0]->werSlice.value()[0]
					<< "\%, slice LER[0]: " << altMeters[0]->lerSlice.value()[0]
					<< ", WER[31]: " << altMeters[MaxAlt - 1]->wer.value()[0]
					<< "\%, LER[31]: " << altMeters[MaxAlt - 1]->ler.value()[0]
					<< "\%, slice WER[31]: " << altMeters[MaxAlt - 1]->werSlice.value()[0]
					<< "\%, slice LER[31]: " << altMeters[MaxAlt - 1]->lerSlice.value()[0]
					<< "\%, progress: "
	...
	buffer << "[Decode " << FLAGS_test << " (" << totalSamples << " samples) in "
			...
			<< totalTime / totalSamples << "s/sample) -- WER: "
			<< std::setprecision(6) << totalWer << ", LER: " << totalLer << "]"
			<< std::endl;
	LOG(INFO) << buffer.str();
	if (!FLAGS_sclite.empty())
	{
